...
\begin{abstract}
  ...
  definici\'on de un procedimiento sistem\'atico para la aplicaci\'on sistem\'atica
  de las mismas.
...
en una aplicacion, la mejora nunca va a superar 1.10x la original. En el caso de tener un 95\%, la mejora nunca va a ser mejor a 20x. Es poco frecuente encontrar
aplicaciones que sean completamente paralelizables, la mayoria solo muestra beneficios relevantes al ser escalada hasta 1024 procesadores.
...
Similarmente al cuadro anterior, podemos deducir que en el caso de un programa con s\'olo 10\% de paralelismo,
...
La fraccion serial {\it Karp-Flatt} $ e $ es determinada experimentalmente, mientras menor sea $ e $ mayor se supone el nivel de paralelismo posible.

Para un problema de tama\~no fijo, la eficiencia tipicamente disminuye cuando el numero de procesadores aumenta.
Se puede entonces determinar si esta disminucion es debida a un paralelismo limitado, a un algoritmo no optimizado o un problema de arquitectura del sistema.
...
pipeline, y work-stealing. En el dise\~no master/worker, un proceso dirige el
trabajo de los dem\'as. En pipeline el trabajo se divide en etapas
independendientes. En work-stealing cada trabajador obtiene independientemente el trabajo
cuando se libera.
...
Intel Threading Building Blocks es un claro ejemplo.
