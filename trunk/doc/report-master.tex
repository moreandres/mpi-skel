...
\begin{enumerate}
\item Analizar el estado del arte del análisis de rendimiento y las herramientas utlizadas para ello.
...
Suponiendo una distribución normal de resultados, se suele controlar que haya menos de 3 $ \sigma $ de diferencia. Se busca que la mayoría de los resultados queden cerca de su promedio, como muestra la Figura \ref{fig:deviation}.
...
Las limitaciones {\it a-priori} que posee la infrastructura son en materia de portabilidad y aplicación. Por el lado de la portabilidad, solo se soportan sistemas GNU/Linux recientes, siendo necesarias el conjunto de utilidades de soporte especificadas anteriormente. Por el lado de la aplicación, solo se soportan programan utilizando tecnología OpenMP multi-hilo.
...
Un parámetro necesario es el rango de tamaños de problema a utilizar durante las ejecuciones, definidos como una secuencia de formato compatible con {\tt seq(1)}.
...
La implementación se desarrollo sobre una plataforma {\it GNU/Linux}, utilizando la distribución {\tt Ubuntu 14.04.1 LTS}.
El lenguaje utilizado Python 2.7.6, publicado en {\it Python Software Foundation Package Index} como hotspot versión 0.3.
El repositorio de código se encuetra en \href{https://github.com/moreandres/hotspot}{https://github.com/moreandres/hotspot}.
La licencia del código fuente es GPLv2.
Se reutilizan librerías como {\tt matplotlib.pyplot} \cite{matplotlib} y {\tt numpy} \cite{numpy} para gráficar las estadísticas obtenidas durante las pruebas de rendimiento.

\section{Reporte Generado}

A continuación se explica el contenido de cada sección en particular.
En el apéndice se muestran tres ejemplos utilizando núcleos de cómputo conocidos.
...
\item Inclusión de una explicación breve del objetivo de la sección y los gráficos incluídos.
...
Contiene un índice de secciones en formato de dos columnas con hipervinculos para facilitar la búsqueda rápida de la información.
...
Además se incluye información de referencia obtenida de la prueba de rendimiento {\it HPCC}.
...
Se incluye un resumen estadistico de los tiempos de ejecución luego de repetir el caso de pruebas varias veces.
Un histograma muestra la distribución de los resultados.
...
Se incluye el reporte de contadores de {\it hardware} relacionados con el rendimiento.
...
El programa resuelve el problema de posicionar en un tablero de ajedrez ocho reinas sin que alguna amenace a las demás. Una reina amenaza a cualquier otra
pieza en el tablero que este en la misma diagonal, fila o columna.
